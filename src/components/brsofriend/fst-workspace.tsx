...
import { Download, FileCheck2, FilePlus2, History, LoaderCircle, Plus, Trash2, Upload } from "lucide-react";
...
import { SavedFstFolder } from "./saved-fst-folder";
  ...
  return (
    <>
      <section className="mt-10 w-full rounded-[1.5rem] border border-[#e1e5dc] bg-white/70 p-5 text-left shadow-xl shadow-[#263d2c]/5 sm:p-7">
        ...
        {/* 사용자별 표시 옵션 */}
        <SettingsPanel settings={uiSettings} onChange={updateUiSetting} />

        {/* 에디터 소개, 기본 파일명, 생성 및 디버그 도구 */}
        <div className="flex flex-col gap-5">
          ...
          {uiSettings.showDefaultName && <label className="max-w-md text-xs font-bold uppercase tracking-wider text-[#72776f]">{t("defaultFileName")}
            ...
            <span className="mt-1.5 block font-normal normal-case tracking-normal text-[#8c9189]">{t("defaultFileNameHint")}</span>
          </label>}

          <div className="flex w-full flex-col gap-3">
            <div className="sm:w-56"><Button type="button" size="lg" onClick={startDefault} disabled={isReading} className="h-11 w-full rounded-full bg-[#5b7f44] px-5 text-white hover:bg-[#4b6c39]"><FilePlus2 /> {t("createFst")}</Button></div>
            {uiSettings.showDebugTools && <div className="flex flex-col gap-3 rounded-xl bg-[#202823] p-3 sm:flex-row sm:items-center">
              ...
              <div className="grid flex-1 grid-cols-2 gap-2">
                <Button type="button" size="lg" variant="outline" onClick={() => inputRef.current?.click()} disabled={isReading} className="h-10 rounded-full border-dashed border-white/25 bg-white/10 px-5 text-white hover:bg-white/20 hover:text-white">{isReading ? <LoaderCircle className="animate-spin" /> : <Upload />} {isReading ? t("reading") : t("uploadFst")}</Button>
        ...
        {settings && <div className="mt-5 space-y-5">
          ...
          <EditorActionBar disabled={!templateBuffer || hasErrors} hasTemplate={Boolean(templateBuffer)} onSave={saveCurrentFst} onExport={handleFstExport} />
        </div>}
      </section>

      {/* 여러 FST를 세션에 보관하고 ZIP으로 내보내는 별도 영역 */}
      <SavedFstFolder items={savedFsts} onLoad={loadSavedFst} onRemove={(name) => setSavedFsts((current) => current.filter((saved) => saved.name !== name))} onExport={exportFstFolder} />
    </>
  );
}
